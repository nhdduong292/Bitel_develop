...
public class UsbDeviceDataExchangeImpl {
    ...
    public String getSerial() {
        if (usb_ctx != null && usb_ctx.mDevConnetion_ != null) {
            return usb_ctx.mDevConnetion_.getSerial();
        } else {
            return null;
        }
    }
    ...
    public static boolean IsFutronicDevice(int idVendor, int idProduct) {
        boolean res = false;
                ...
                // first line in if() is test, the fingerprint scanner at Bitel is idVendor == 1839 and idProduct = 37068
        if (
    ...
    private FTR_USB_DEVICE_INTERNAL OpenDevice(UsbDevice device) {
        ...
        if (intf != null) {
            ...
            Log.e(log_tag, "Get interfaces failed failed in device: " + device);
        }
