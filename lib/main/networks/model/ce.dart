...
class CE extends IdentifyModel {
  ...
  void startDetect(List<String> documentItems) {
    if (documentItems.length > 1) {
      ...
      if (checkForCorrectType(documentItems[0], documentItems[1])) {
        setupDocumentTwoLine(documentItems, nationalOffset);
        setupDocumentThreeLine(documentItems, 0);
      } else {}
    }
  }
  ...
  void omitRedundantItems(List<String> firstDocument) {
    ...
    String CEVEN = "CEVEN";
    String CE = "C<";
    if (first.contains(CEVEN) || first.contains(CE)) {
      String startWith = "";
      if (first.contains(CEVEN))
        startWith = CEVEN;
      else if (first.contains(CE)) startWith = CE;

      int start = first.indexOf(startWith);
  ...
  bool checkForCorrectType(String? first, String? second) {
    if (first != null && second != null) {
      nationalOffset =
          (first.startsWith("9C") || first.startsWith("GC")) ? -1 : 0;
      return ((first.startsWith("C") ||
                  first.startsWith("9C") ||
                  first.startsWith("GC")) &&
              second.contains(getNationalOfFirst(first, nationalOffset))) ||
          first.startsWith("CE");
    } else
      return false;
  }
